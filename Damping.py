...
def runge_kutta(f,t0,x0,dt,dxdt_0,t_max,b,w):
	T = []; X = []; V = []; n = 4
	a = [0,0.5,0.5,1];k = [[0 for j in range(n)] for i in range(2)]
	while abs(t0)<abs(t_max):
		...
		X.append(x0); T.append(t0); V.append(dxdt_0)
...
sol_T, sol_X, sol_v = runge_kutta(f,t0,x0,dt,dxdt_0,t_max,b,w)
if (b**b)-(w*w)>0:
	typee = "Dead beat Type"
elif (b==w):
	typee = "Critical damping"
else:
	typee = "Small Damping"

plt.plot(sol_T,sol_X,c='red',label='b = %s ;w = %s \n t0 = %s ; x0 = %s ; v0 = %s'%(b,w,t0,x0,dxdt_0))
#plt.plot(sol_T,sol_v,ls='--',c='green',label='velocity')

plt.legend(loc='best',prop={'size':12})
plt.xlabel("time")
plt.ylabel("Amplitude and Velocity")
plt.title(" damping curve(Type:%s)"%(typee))
plt.show()